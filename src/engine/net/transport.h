...
class TransportAddress {
  public:
    ...
    static TransportAddress fromPortAnyAddress(unsigned short port) { return TransportAddress(port); }

    static TransportAddress fromRaw(sockaddr_in addr) { return TransportAddress(addr); }
...
class ReceiveBytes {
  public:
    ...
    ReceiveBytes(uint8_t* inBytes, int inLen) : bytes(inBytes), len(inLen) {}
