...
unsigned short TransportAddress::port() const { return ntohs(this->addr_.sin_port); }
...
ReceiveBytes::ReceiveBytes(ReceiveBytes&& other) noexcept : bytes(other.bytes), len(other.len) {
    other.bytes = nullptr;
    other.len = 0;
}
